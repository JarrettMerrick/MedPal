...
const VERSION_HISTORY: ChangelogEntry[] = [
  {
    version: 'v1.2.3',
    date: '2026-09-16',
    codename: '时间口径统一 + 会话安全加固 + 品牌清理',
    changes: [
      '【时间显示】全站时间口径统一：后端接口时间统一输出为带时区标记的 UTC（如 2026-09-16T07:27:03Z），前端一律经统一时间工具按浏览器时区转换显示；修复标识维修时间、系统日志、导出内容等"显示比北京时间少 8 小时"的问题。',
      '【时间计算】按日期筛选（系统日志、巡检历史、维修记录）统一按北京业务日期换算查询边界，修复北京时间 00:00-08:00 数据漏筛；Excel/CSV 导出与下载文件名统一使用北京时间。',
      '【会话安全】修复改密/重置密码后旧令牌未被及时吊销的问题（令牌缺少内嵌改密时间时跳过校验、时间比对存在 8 小时时区盲区），并补齐文件下载与静态文件接口的失效校验；本人改密成功后为当前会话重新签发令牌，不再被强制下线。',
      '【品牌统一】清理代码、注释与部署配置中的历史机构品牌信息；内置默认 Logo 改为代码绘制的默认样式（不再依赖位图文件），系统名称默认值统一为 MedPal。',
      '【仓库维护】修正 .gitignore 规则（部分规则未锚定根目录，导致源码目录被静默排除、新克隆仓库构建失败）；部署配置目录纳入版本管理。',
      '【其他】版本号升级至 v1.2.3。',
    ],
  },
  {
    version: 'v1.2.2',
    date: '2026-09-16',
    codename: '重置密码规则统一 + 运维 CLI',
    changes: [
      '【用户管理】“重置密码”逻辑统一采用账户设置中配置的默认口令模板（如 MedPal@2026，支持 {工号} 占位符），移除原先独立的硬编码规则（固定前缀+工号），保证重置后的口令与账户规则一致。',
      '【运维】新增 backend/manage.py 运维命令行，支持 reset-admin（重置管理员密码）与 list-admins（列出超级管理员），便于服务器侧运维。',
      '【其他】版本号升级至 v1.2.2。',
    ],
  },
  {
