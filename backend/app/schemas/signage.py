...
class SignageBase(BaseModel):
    ...
    # [新增 2026-09-17] 文件库关联：标识编辑页「从标准库选择」时提交被引用的设计文件ID
    # （引用共享；design_photo 路径仍由上传/选择逻辑写入，保持既有链路不变）
    design_file_id:Optional[int]=None
    # [新增 2026-09-17] 设计文件路径：既可由上传接口写入，也支持「从标准库选择」时直接写入
    # （选择文件库中的文件等于引用其路径，与更新接口口径一致）
...
class SignageResponse(SignageBase):
    ...
    # [新增 2026-09-17] 引用的设计文件在「文件管理」中的**使用名**（DesignFile.name），
    # 由 service 按 design_file_id 关联填充。前端展示设计文件时优先用它，
    # 避免显示 design_photo 路径里的原始落盘名（含时间戳随机串，对使用者无意义）；
    # 未引用文件库（design_file_id 为空）时为 None，前端回退到路径文件名。
